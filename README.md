...
3. https://www.kaggle.com/datasets/mkechinov/ecommerce-purchase-history-from-jewelry-store
...
A third dataset a jewelry eCommerce interaction log spanning three years across 25 unique customers was additionally evaluated. While the Segmented XGBoost remained the strongest performer on this dataset (R² = 0.9049), the dataset was excluded from the primary cross-dataset comparison due to its extremely limited customer base, the Category XGBoost could not be applied as no product category contained sufficient samples and both LSTM-based models produced negative R² values confirming that deep sequence architectures require a minimum training population that this dataset could not satisfy.
